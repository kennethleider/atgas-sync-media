...
class DNSSDUnitTest {
    ...
    public void testTxtRecord() {
        ...
        System.out.println((new String(rawBytes, 0, rawBytes.length))
                + " has count " + String.valueOf(txtRecord.size()));

        System.out.println(txtRecord);
        boolean ttlPresent = txtRecord.contains("ttl");
        System.out
                .println("ttl is present: " + (ttlPresent ? "true" : "false"));
        boolean timeoutPresent = txtRecord.contains("timeout");
        System.out.println("timeout is present: "
                + (timeoutPresent ? "true" : "false"));
    ...
    public void run() throws DNSSDException {
        System.out.println("Running DNSSD unit test for "
...
class TermReporter implements BaseListener {
    public void operationFailed(DNSSDService service, int errorCode) {
        System.out.println(this.getClass().getName() + " encountered error "
...
class RegTest extends TermReporter implements RegisterListener {
    ...
    public RegTest() throws DNSSDException {
        fReg = DNSSD.register(0, 0, "Test service", DNSSDUnitTest.TEST_TYPE,
                "", "", TEST_PORT, null, this);
    }
    ...
    public void serviceRegistered(DNSSDRegistration registration, int flags,
        ...
        try {
            ...
            byte[] kResponsiblePerson = { 'c', 'o', 'o', 'k', 'i', 'e', ' ',
                    'm', 'o', 'n', 's', 't', 'e', 'r' };
...
class DupRegTest extends TermReporter implements RegisterListener {
    ...
    public DupRegTest() throws DNSSDException {
        DNSSD.register(DNSSD.NO_AUTO_RENAME | DNSSD.UNIQUE, 0, "Test service",
                DNSSDUnitTest.TEST_TYPE, "", "", TEST_PORT + 1, null, this);
    }
...
class DomainTest extends TermReporter implements DomainListener {
    ...
    public void domainFound(DNSSDService enumerator, int flags, int ifIndex,
            String domain) {
        String s = "Domain found flags:" + String.valueOf(flags) + " ifIndex:"
                + String.valueOf(ifIndex) + " domain:" + domain;
        System.out.println(s);
    }

    public void domainLost(DNSSDService enumerator, int flags, int ifIndex,
            String domain) {
        String s = "Domain lost flags:" + String.valueOf(flags) + " ifIndex:"
                + String.valueOf(ifIndex) + " domain:" + domain;
        System.out.println(s);
    }
    ...
    public void operationFailed(DNSSDService service, int errorCode) {
        System.out
                .println("Domain enum op failed " + String.valueOf(errorCode));
    }
}

class ResolveTest extends TermReporter implements ResolveListener {
    public ResolveTest(int flags, int ifIndex, String serviceName,
    ...
    public void serviceResolved(DNSSDService resolver, int flags, int ifIndex,
        ...
        for (int i = 0; null != (a = txtRecord.getKey(i)); i++)
            s += " attr/val " + String.valueOf(i) + ": " + a + ","
                    + txtRecord.getValueAsString(i);
...
class QueryTest extends TermReporter implements QueryListener {
    public QueryTest(int flags, int ifIndex, String serviceName, int rrtype,
            int rrclass) {
        try {
            DNSSD.queryRecord(flags, ifIndex, serviceName, rrtype, rrclass,
                    this);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void queryAnswered(DNSSDService query, int flags, int ifIndex,
        ...
        String s = "QueryTest result flags:" + String.valueOf(flags)
                + " ifIndex:" + String.valueOf(ifIndex) + " fullName:"
                + fullName + " rrtype:" + String.valueOf(rrtype) + " rrclass:"
                + String.valueOf(rrclass) + " ttl:" + String.valueOf(ttl);
        System.out.println(s);

        try {
            String dataTxt = new String(rdata, 0, rdata.length,
                    DNSSDUnitTest.WIRE_CHAR_SET);
    ...
    public RegistrarTest() {
        try {
            ...
            fRegistrar.registerRecord(DNSSD.UNIQUE, 0,
                    "test.registrartest.local", 17 /* ns_t_rp */, 1,
                    kResponsiblePerson, 3600);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
